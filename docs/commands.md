...
- 2026-03-03: 消除 libflowsql_framework.so — 回归纯插件架构：新建 common/iplugin.h 拆分接口；IPlugin::Load(IQuerier*) 签名变更；Scheduler 通道管理内部化；删除 PluginRegistry；flowsql_framework → flowsql_common；Pipeline/ChannelAdapter 移入 scheduler.so
- 2026-03-03: Bridge 算子注册机制改进 — IBridge 接口方案：新建 ibridge.h 纯接口（FindOperator+Refresh）；BridgePlugin 多继承 IBridge，删除 dynamic_cast<IRegister> hack；plugin_register.cpp 改用宏注册 IID_PLUGIN+IID_BRIDGE；Scheduler FindOperator 返回 shared_ptr 先查 C++ 再查 IBridge，新增 /refresh-operators 端点；Web 激活/停用后调用 NotifySchedulerRefresh 通知刷新链路
- 2026-03-03: IChannel/IOperator 去掉 IPlugin 继承 — 纯接口不应继承生命周期接口：ichannel.h/ioperator.h 改为独立接口（加 virtual ~析构）；MemoryChannel/PassthroughOperator 显式多继承 IPlugin；DataFrameChannel/PythonOperatorBridge 删除 IPlugin 方法桩（内部使用不需要）；example/plugin_register.cpp 改用 static_cast 替代 dynamic_cast
