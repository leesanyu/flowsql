...
所有操作统一使用 `POST` + JSON body，资源定位通过 body 中的标识字段（如 `type`+`name`），不使用路径参数。
...
|------|------|
| `GET /api/health` | 健康检查 |
| `GET /api/channels/list` | 查询通道列表（代理到 Scheduler） |
| `GET /api/operators/list` | 查询算子列表（代理到 Scheduler） |
| `POST /api/operators/upload` | 上传 Python 算子文件 |
| `POST /api/operators/activate` | 激活算子 |
| `POST /api/operators/deactivate` | 停用算子 |
| `GET /api/tasks/list` | 查询任务列表 |
| `POST /api/tasks/submit` | 提交 SQL 任务（代理到 Scheduler） |
...
| 端点 | 说明 |
|------|------|
| `POST /operators/native/query` | 查询 C++ 算子（SchedulerPlugin） |
...
| `POST /operators/python/work/{catelog}/{name}` | 执行算子（Bridge 直连调用） |
| `POST /operators/python/configure/{catelog}/{name}` | 配置算子参数 |
