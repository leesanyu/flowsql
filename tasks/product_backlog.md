...
| [ ] | NPM 通用参数入口 (`npm-basic-parameters`) | P1 | 让同一 `npm.basic` 任务既需要共享配置又组合多个模块的用户，不再为每类能力增加专用顶层 `WITH` 参数或承受配置归属冲突；交付单一 `parameters` JSON 字符串入口，以 `task` 命名空间承载应用范围等任务共享配置、以 `modules` 命名空间按已启用的 `features` 承载模块私有配置，统一校验未知/重复/禁用模块参数并在任务创建时冻结类型化配置；保留旧 Basic/Session SQL 兼容，不交付具体配置内容的业务解释。 | 待创建 |
