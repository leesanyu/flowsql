...
| [-] | NPM 流量标签化 (`npm-labeling`) | P1 | 依赖 `config-channel` 与 `npm-basic-parameters`；让中大型网络中需要按 MAC、VLAN、IP/CIDR、传输协议和端口确定会话流主归属的用户，不再由各模块重复匹配规则或面对多结果冲突；交付独立同进程能力插件 `libflowsql_npm_labeling.so`，由 `features` 显式启用的任务通过固定 IID 绑定精确配置快照和任务私有 DPDK ACL matcher，为每个双向会话流产生零或一个唯一具名主标签，供结果投影、模块选择和早期处理准入使用；插件统一拥有进程级 EAL 与 DPDK 依赖，未部署插件或未启用标签化不影响既有 Basic/Session，启用但 provider/运行环境不可用则在任务打开前明确失败。 | [规格](specs/feat-npm-labeling.md) |
| [ ] | NPM 共享有界 TCP 字节流 (`npm-shared-tcp-stream`) | P1 | 依赖 `npm-basic-parameters` 与 `npm-labeling`；为同一 `npm.basic` 任务内的多个 TCP 应用协议模块解决各自处理 sequence、重传、重叠、乱序与缺口造成的重复状态和结果分歧；交付仅为命中模块所选早期唯一主标签的会话按方向共享、按需创建且任务预算有界的 TCP 有序字节视图，明确连续区间、缺口、截断与会话终结事实；不默认重组全部 TCP，不模拟完整 TCP 端点或解析具体应用协议。 | 待创建 |
| [ ] | NPM 协议模块运行时基础 (`npm-protocol-analysis`) | P1 | 依赖 `npm-basic-parameters`；为在同一 `npm.basic` 任务中组合多个协议分析能力的模块开发者，解决各模块重复解析 packet、另建会话上下文并各自处理生命周期与异构结果造成的重复计算和资源不可控；交付统一的 `features` 启用、`observing` 观察、packet/datagram/session 事件分发、公共唯一主标签传递、结果实体生命周期、任务预算和类型化路由，使多个已启用模块只执行一次基础解码、会话化与协议识别；使用标签准入的模块必须显式声明对 `npm-labeling` 的依赖，本 Feature 不交付具体 DNS、HTTP、TLS 或 ICMP 解析器。 | 待创建 |
