...
bool IsBareSyn(const NpmTcpPacketFacts& tcp) noexcept { return tcp.syn && !tcp.ack; }
...
struct NpmTcpPerformanceTracker::Impl {
    ...
    static uint64_t SessionCharge() noexcept { return sizeof(SessionMap::value_type) + 4 * sizeof(void*); }

    static uint64_t SegmentCharge() noexcept { return sizeof(OutstandingSegment) + 2 * sizeof(void*); }
    ...
    void InitializeHandshake(SessionState* state, const NpmPacketView& packet, int64_t timestamp_ns) noexcept {
    ...
    void ObserveHandshake(SessionState* state, const NpmPacketView& packet) noexcept {
        ...
        if (state->initial_syn_observed && IsBareSyn(tcp)) {
            if (packet.direction == state->initial_syn_direction && tcp.sequence == state->initial_syn_sequence) {
        ...
        if (state->handshake_status == NpmTcpHandshakeStatus::kAmbiguous ||
            state->handshake_status == NpmTcpHandshakeStatus::kNotObserved || !state->initial_syn_observed) {
            return;
        }
    ...
    void ObserveAcknowledgment(SessionState* state, DirectionState* direction, int64_t acknowledgment,
    ...
    NpmTcpPerformanceError ObserveRanges(SessionState* state, const NpmPacketView& packet) noexcept {
        ...
        const auto sequence =
            outgoing.initialized ? Unwrap(tcp.sequence, outgoing.progress) : std::optional<int64_t>(tcp.sequence);
        ...
        const auto duplicate = std::find_if(outgoing.outstanding.begin(), outgoing.outstanding.end(),
                                            [&](const OutstandingSegment& segment) {
                                                return segment.sequence_begin == begin && segment.sequence_end == end;
                                            });
        const bool regressed = state->timestamp_regressed || packet.packet.meta.timestamp_ns < state->max_timestamp_ns;
        const bool new_history = has_payload && merged_ranges == 0;
        const bool new_outstanding = has_payload && !regressed && state->rtt_status != NpmTcpRttStatus::kAmbiguous &&
                                     duplicate == outgoing.outstanding.end() &&
                                     (!outgoing.highest_ack.has_value() || end > *outgoing.highest_ack);
        const size_t history_count =
            outgoing.history.size() + static_cast<size_t>(new_history) - (merged_ranges > 0 ? merged_ranges - 1 : 0);
        const size_t total_ranges = history_count + outgoing.outstanding.size() + static_cast<size_t>(new_outstanding);
    ...
    static bool CanCountPacket(const SessionState& state, const NpmPacketView& packet) noexcept {
        ...
        return CounterCanAdd(ab ? state.packets_ab : state.packets_ba, 1) &&
               ...
               CounterCanAdd(ab ? state.payload_bytes_ab : state.payload_bytes_ba, packet.transport.payload_wire_bytes);
    }
    ...
    NpmTcpPerformanceError ObserveNew(const NpmPacketView& packet, uint64_t session_id, uint8_t protocol) noexcept {
...
NpmTcpPerformanceTracker::~NpmTcpPerformanceTracker() { Clear(); }

NpmTcpPerformanceError NpmTcpPerformanceTracker::Observe(const NpmPacketView& packet, const NpmSessionView& session) {
...
NpmTcpPerformanceError NpmTcpPerformanceTracker::Snapshot(const NpmSessionView& session,
                                                          NpmTcpPerformanceSnapshot* output) const {
    if (!output) return NpmTcpPerformanceError::kNullOutput;
    ...
    if (!duration.has_value() || session.first_ns > state.min_timestamp_ns ||
        ...
        session.packets_ba < state.packets_ba || session.wire_bytes_ab < state.observed_wire_bytes_ab ||
...
size_t NpmTcpPerformanceTracker::size() const noexcept { return impl_->sessions.size(); }
...
uint64_t NpmTcpPerformanceTracker::tracked_bytes() const noexcept { return impl_->tracked_bytes; }
