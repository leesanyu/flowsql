...
NpmSessionAnalysisModule::NpmSessionAnalysisModule(const NpmProtocolContext& protocol_context,
    ...
    : protocol_context_(protocol_context), budget_(std::move(budget)), tracker_(budget_, max_ranges_per_direction) {}

NpmSessionAnalysisModule::~NpmSessionAnalysisModule() { Clear(); }

int NpmSessionAnalysisModule::OnPacket(const NpmPacketView& packet, const NpmSessionView& session,
                                       INpmResultWriter& writer) {
    (void)writer;
    return TrackerErrorToErrno(tracker_.Observe(packet, session));
}

int NpmSessionAnalysisModule::OnSessionSnapshot(const NpmSessionView& session, int64_t observed_at_ns,
                                                INpmResultWriter& writer) {
    return Emit(session, false, NpmSessionEndReason::kEof, observed_at_ns, writer);
}

int NpmSessionAnalysisModule::OnSessionEnd(const NpmSessionView& session, NpmSessionEndReason reason,
                                           int64_t observed_at_ns, INpmResultWriter& writer) {
    return Emit(session, true, reason, observed_at_ns, writer);
}

int NpmSessionAnalysisModule::Emit(const NpmSessionView& session, bool is_final, NpmSessionEndReason reason,
                                   int64_t observed_at_ns, INpmResultWriter& writer) {
    ...
    const uint64_t duration = static_cast<uint64_t>(session.last_ns) - static_cast<uint64_t>(session.first_ns);
...
size_t NpmSessionAnalysisModule::tracked_sessions() const noexcept { return tracker_.size(); }

uint64_t NpmSessionAnalysisModule::tracked_bytes() const noexcept { return tracker_.tracked_bytes() + revision_bytes_; }
