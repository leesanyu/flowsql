...
NpmPacketBatchError ValidateIp(int64_t row,
                               const char* prefix,
                               const arrow::UInt8Array& family,
                               const arrow::UInt32Array& v4,
                               const arrow::BinaryArray& v6,
                               std::string* error) {
        ...
        case packet::AddressFamily::kIPv6:
            ...
            if (has_v6 && v6.GetView(row).size() != sizeof(packet::IPv6Address)) {
                return Fail(NpmPacketBatchError::kInvalidRow,
                            std::string(prefix) + "_ip_v6 must contain 16 bytes",
                            error);
            }
            break;
    }
    return Fail(NpmPacketBatchError::kInvalidRow,
                std::string(prefix) + "_ip family does not match nullable address columns",
                error);
}

}  // namespace

NpmPacketBatchError NpmPacketBatchView::ValidateRow(int64_t row,
                                                    const NpmPacketBatchView::Columns& columns,
                                                    std::string* error) {
    int32_t raw_size = 0;
    ...
    auto result = ValidateIp(row,
                             "src",
                             *columns.src_ip_family,
                             *columns.src_ip_v4,
                             *columns.src_ip_v6,
                             error);
    if (result != NpmPacketBatchError::kNone) return result;
    result = ValidateIp(row,
                        "dst",
                        *columns.dst_ip_family,
                        *columns.dst_ip_v4,
                        *columns.dst_ip_v6,
                        error);
    if (result != NpmPacketBatchError::kNone) return result;
    ...
    if ((ports_valid && (!has_src_port || !has_dst_port)) ||
        (!ports_valid && (has_src_port || has_dst_port))) {
    ...
    if ((identified && (!has_protocol_id || !has_protocol_sub_id)) ||
        ...
        return Fail(NpmPacketBatchError::kInvalidRow,
                    "protocol_status does not match nullable protocol ID columns",
                    error);
    }
    return NpmPacketBatchError::kNone;
}
...
NpmPacketBatchView::NpmPacketBatchView(std::shared_ptr<arrow::RecordBatch> batch,
...
NpmPacketBatchError NpmPacketBatchView::Create(const std::shared_ptr<arrow::RecordBatch>& batch,
    ...
    if (!validation.ok()) {
        return Fail(NpmPacketBatchError::kInvalidColumn,
                    std::string("packet batch Arrow validation failed: ") + validation.ToString(),
                    error);
    }
    ...
    try {
        ...
        columns->layer_id_values =
            std::static_pointer_cast<arrow::UInt16Array>(columns->layer_ids->values());
        columns->layer_offsets = PacketColumnAs<arrow::FixedSizeListArray>(batch, kLayerOffsets);
        columns->layer_offset_values =
            std::static_pointer_cast<arrow::UInt32Array>(columns->layer_offsets->values());
        ...
        auto view = std::unique_ptr<NpmPacketBatchView>(
            new NpmPacketBatchView(batch, std::move(columns)));
...
int64_t NpmPacketBatchView::num_rows() const {
    return batch_->num_rows();
}

NpmPacketBatchError NpmPacketBatchView::Get(int64_t row,
    ...
    auto decode_ip = [row](const arrow::UInt8Array& family,
                           const arrow::UInt32Array& v4,
                           const arrow::BinaryArray& v6,
    ...
    decode_ip(*columns_->src_ip_family,
              *columns_->src_ip_v4,
              *columns_->src_ip_v6,
              &next_layer.src_ip);
    decode_ip(*columns_->dst_ip_family,
              *columns_->dst_ip_v4,
              *columns_->dst_ip_v6,
              &next_layer.dst_ip);
