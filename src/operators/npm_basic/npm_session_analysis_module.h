...
class NpmSessionAnalysisModule final : public INpmAnalysisModule {
 public:
    NpmSessionAnalysisModule(const NpmProtocolContext& protocol_context,
    ...
    int OnPacket(const NpmPacketView& packet,
                 const NpmSessionView& session,
                 INpmResultWriter& writer) override;
    int OnSessionSnapshot(const NpmSessionView& session,
                          int64_t observed_at_ns,
                          INpmResultWriter& writer) override;
    int OnSessionEnd(const NpmSessionView& session,
 ...
 private:
    int Emit(const NpmSessionView& session,
             bool is_final,
             NpmSessionEndReason reason,
             int64_t observed_at_ns,
