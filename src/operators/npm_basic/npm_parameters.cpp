...
NpmParameterStatusV1 ValidateAnalysisRange(const NpmAnalysisConfig& config) {
    switch (ValidateNpmAnalysisConfig(config)) {
        ...
        case NpmAnalysisConfigError::kInvalidRunMode:
            return Fail(NpmParameterErrorV1::kInvalidValue, "/framework/run_mode");
        case NpmAnalysisConfigError::kInvalidResultMode:
            return Fail(NpmParameterErrorV1::kInvalidValue, "/framework/result_mode");
        case NpmAnalysisConfigError::kOutputIntervalOutOfRange:
            return Fail(NpmParameterErrorV1::kInvalidRange, "/framework/output_interval_ns");
        case NpmAnalysisConfigError::kPayloadSamplePacketsOutOfRange:
            return Fail(NpmParameterErrorV1::kInvalidRange, "/framework/payload_sample_packets");
        case NpmAnalysisConfigError::kTcpIdleTimeoutOutOfRange:
            return Fail(NpmParameterErrorV1::kInvalidRange, "/framework/tcp_idle_timeout_ns");
...
NpmParameterStatusV1 ParseFramework(const rapidjson::Value* value,
    ...
    status = ReadString(*value, "run_mode", "/framework", &text);
    if (status.error != NpmParameterErrorV1::kNone) return status;
    if (text) {
    ...
    status = ReadString(*value, "result_mode", "/framework", &text);
    if (status.error != NpmParameterErrorV1::kNone) return status;
    if (text) {
        const std::string_view mode(text->GetString(), text->GetStringLength());
        if (mode == "final") {
    ...
    status = ReadString(*value, "overload_policy", "/framework", &text);
    if (status.error != NpmParameterErrorV1::kNone) return status;
    if (text) {
    ...
    if (consumers.labeling_enabled && consumers.labeling_available) {
        status = ReadString(*value, "labeling", "/framework", &text);
        if (status.error != NpmParameterErrorV1::kNone) return status;
        if (text) {
...
NpmParameterStatusV1 ValidateModuleNodes(const rapidjson::Value& root) {
    ...
    for (auto member = root.MemberBegin(); member != root.MemberEnd(); ++member) {
        const std::string_view name(member->name.GetString(), member->name.GetStringLength());
        if (name == "schema_version" || name == "framework") continue;
...
NpmParameterStatusV1 ParseNpmParametersV1(const char* parameters_json,
    ...
    try {
        ...
        status = ParseFramework(Find(document, "framework"), consumers, &next.framework);
        if (status.error != NpmParameterErrorV1::kNone) return status;
