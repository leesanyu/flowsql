...
NpmBasicEncodeError EncodeNpmBasicResults(const std::vector<NpmBasicResult>& results,
    ...
    try {
        ...
        auto append_optional_uint16 = [&](const std::optional<uint16_t>& value, arrow::UInt16Builder* builder,
                                          const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        auto append_optional_string = [&](const std::optional<std::string>& value, arrow::StringBuilder* builder,
                                          const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
...
NpmBasicEncodeError EncodeNpmBasicResultsWithBudget(const std::vector<NpmBasicResult>& results,
    ...
    try {
        ...
        if (signed_bytes < 0) {
            return Fail(NpmBasicEncodeError::kInvalidBufferSize, "encoded batch has a negative Arrow buffer size",
                        error);
        }
        ...
        *output = std::move(leased_batch);
        ...
        return Fail(NpmBasicEncodeError::kAllocationFailed, "NPM basic pending output allocation failed", error);
    }
}
