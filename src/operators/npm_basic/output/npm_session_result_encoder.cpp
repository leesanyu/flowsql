...
NpmSessionEncodeError Fail(NpmSessionEncodeError code, const std::string& message, std::string* error) {
...
class PendingOutputOwner {
 public:
    PendingOutputOwner(std::shared_ptr<arrow::RecordBatch> batch, std::shared_ptr<INpmTaskBudget> budget,
                       uint64_t bytes)
...
NpmSessionEncodeError EncodeNpmSessionResults(const std::vector<NpmSessionResult>& results,
    ...
    try {
            ...
            if (validation != NpmSessionResultError::kNone) {
                return Fail(NpmSessionEncodeError::kInvalidResult,
                            "result[" + std::to_string(index) +
                                "] is invalid: " + std::to_string(static_cast<uint8_t>(validation)),
                            error);
            }
            std::string invalid_utf8_field;
            if (!ValidResultUtf8(results[index], &invalid_utf8_field)) {
                return Fail(NpmSessionEncodeError::kInvalidResult,
                            "result[" + std::to_string(index) + "]." + invalid_utf8_field + " is not valid UTF-8",
                            error);
            }
        }
        ...
        const auto append_optional_uint16 = [&](const std::optional<uint16_t>& value, arrow::UInt16Builder* builder,
                                                const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        const auto append_optional_uint64 = [&](const std::optional<uint64_t>& value, arrow::UInt64Builder* builder,
                                                const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        const auto append_optional_int64 = [&](const std::optional<int64_t>& value, arrow::Int64Builder* builder,
                                               const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        const auto append_optional_double = [&](const std::optional<double>& value, arrow::DoubleBuilder* builder,
                                                const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        const auto append_optional_string = [&](const std::optional<std::string>& value, arrow::StringBuilder* builder,
                                                const char* field) {
            return value.has_value() ? check(builder->Append(*value), field) : check(builder->AppendNull(), field);
        };
        ...
        for (const auto& result : results) {
            if (!check(session_id.Append(result.session_id), "session_id") ||
                ...
                !check(a_port.Append(result.a_port), "a_port") || !check(b_port.Append(result.b_port), "b_port") ||
                ...
                !check(protocol_status.Append(NpmProtocolStatusName(result.protocol_status)), "protocol_status") ||
            ...
            if (!check(packets_ab.Append(result.packets_ab), "packets_ab") ||
                ...
                !append_optional_double(result.payload_bps_ab, &payload_bps_ab, "payload_bps_ab") ||
                !append_optional_double(result.payload_bps_ba, &payload_bps_ba, "payload_bps_ba") ||
                !append_optional_uint64(result.tcp_unique_payload_bytes_ab, &tcp_unique_payload_bytes_ab,
                                        "tcp_unique_payload_bytes_ab") ||
                !append_optional_uint64(result.tcp_unique_payload_bytes_ba, &tcp_unique_payload_bytes_ba,
                                        "tcp_unique_payload_bytes_ba") ||
                !append_optional_double(result.tcp_unique_payload_bps_ab, &tcp_unique_payload_bps_ab,
                                        "tcp_unique_payload_bps_ab") ||
                !append_optional_double(result.tcp_unique_payload_bps_ba, &tcp_unique_payload_bps_ba,
                                        "tcp_unique_payload_bps_ba") ||
                !check(tcp_handshake_status.Append(NpmTcpHandshakeStatusName(result.tcp_handshake_status)),
                       "tcp_handshake_status")) {
                return NpmSessionEncodeError::kArrowError;
            }
            if (result.tcp_initiator.has_value()) {
                if (!check(tcp_initiator.Append(NpmTcpInitiatorName(*result.tcp_initiator)), "tcp_initiator")) {
                    return NpmSessionEncodeError::kArrowError;
                }
            } else if (!check(tcp_initiator.AppendNull(), "tcp_initiator")) {
                return NpmSessionEncodeError::kArrowError;
            }
            if (!append_optional_int64(result.tcp_handshake_duration_ns, &tcp_handshake_duration_ns,
                                       "tcp_handshake_duration_ns") ||
                !append_optional_int64(result.tcp_synack_rtt_ns, &tcp_synack_rtt_ns, "tcp_synack_rtt_ns") ||
                !check(tcp_rtt_status.Append(NpmTcpRttStatusName(result.tcp_rtt_status)), "tcp_rtt_status") ||
                !append_optional_uint64(result.tcp_rtt_samples, &tcp_rtt_samples, "tcp_rtt_samples") ||
                !append_optional_int64(result.tcp_rtt_min_ns, &tcp_rtt_min_ns, "tcp_rtt_min_ns") ||
                !append_optional_int64(result.tcp_rtt_mean_ns, &tcp_rtt_mean_ns, "tcp_rtt_mean_ns") ||
                !append_optional_int64(result.tcp_rtt_max_ns, &tcp_rtt_max_ns, "tcp_rtt_max_ns") ||
                !check(
                    tcp_retransmission_status.Append(NpmTcpRetransmissionStatusName(result.tcp_retransmission_status)),
                    "tcp_retransmission_status") ||
                !append_optional_uint64(result.tcp_retrans_packets_ab, &tcp_retrans_packets_ab,
                                        "tcp_retrans_packets_ab") ||
                !append_optional_uint64(result.tcp_retrans_packets_ba, &tcp_retrans_packets_ba,
                                        "tcp_retrans_packets_ba") ||
                !append_optional_uint64(result.tcp_retrans_payload_bytes_ab, &tcp_retrans_payload_bytes_ab,
                                        "tcp_retrans_payload_bytes_ab") ||
                !append_optional_uint64(result.tcp_retrans_payload_bytes_ba, &tcp_retrans_payload_bytes_ba,
                                        "tcp_retrans_payload_bytes_ba") ||
                !check(measurement_flags.Append(result.measurement_flags), "measurement_flags")) {
                return NpmSessionEncodeError::kArrowError;
            }
        }
        ...
        if (!validation.ok()) {
            return Fail(NpmSessionEncodeError::kArrowError, std::string("encoded batch: ") + validation.ToString(),
                        error);
        }
        *output = std::move(batch);
        ...
        return Fail(NpmSessionEncodeError::kAllocationFailed, "NPM Session result allocation failed", error);
    }
}
...
NpmSessionEncodeError EncodeNpmSessionResultsWithBudget(const std::vector<NpmSessionResult>& results,
    ...
    try {
        ...
        if (signed_bytes < 0) {
            return Fail(NpmSessionEncodeError::kInvalidBufferSize, "encoded batch has a negative Arrow buffer size",
                        error);
        }
        ...
        if (budget_result != NpmBudgetError::kNone) {
            return Fail(NpmSessionEncodeError::kBudgetError,
                        std::string("pending output budget reservation failed: ") + BudgetErrorName(budget_result),
                        error);
        }
        ...
        *output = std::move(leased_batch);
        ...
        return Fail(NpmSessionEncodeError::kAllocationFailed, "NPM Session pending output allocation failed", error);
    }
}
