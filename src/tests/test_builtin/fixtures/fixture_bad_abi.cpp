...
class FixtureBadAbiOperator : public flowsql::IOperator {
 public:
    std::string Category() override { return "cppdemo"; }
    std::string Name() override { return "bad_abi"; }
    std::string Description() override { return "fixture bad abi operator"; }
...
FLOWSQL_SDK_EXPORT flowsql::IOperator* flowsql_create_operator(int index) {
    ...
    return new FixtureBadAbiOperator();
}
