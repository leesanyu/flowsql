...
NpmProtocolContextError NpmProtocolContext::Create(IQuerier* querier,
                                                   std::unique_ptr<NpmProtocolContext>* output) {
    ...
    std::unique_ptr<NpmProtocolContext> context(
        new (std::nothrow) NpmProtocolContext(pool, protocol, dictionary, pipeno));
    if (!context) {
        pool->Release(pipeno);
...
NpmProtocolContext::NpmProtocolContext(IProtocolPipelinePoolV1* pool,
                                       IProtocol* protocol,
